...
Let $W^n_w$ be the set $\{x \in \{0, 1\}^n \mid HW(x) \leq w \}$, where $HW(x)$ is the hamming weight of a vector $x$. 

Let $\binom{n}{\downarrow w} = \sum^w_{i = 0} \binom{n}{i}$. This is also the size of the set $W^n_w$.

The subscript $z_i \leftarrow 0$ implies that a polynomial $p_{z_i \leftarrow 0}(y,z)$ has it's $i$th $z$-bit set to zero.
...
\begin{figure}[h]
    ...
    \begin{alg}
        ...
        \KwResult{A two-dimensional list of size $(n - n_1) \times (n_1 + 1)$ containing the $z_i$ bits, $y$ bits and $U_0(y)$ bit.}
