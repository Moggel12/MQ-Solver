...
\begin{equation*}
    ...
    \end{split}
\end{equation*}
Now, a nice side effect of this definition is that the Möbius transform is an involution, and so we may both obtain the ANF of a boolean function/polynomial $f$ via its truth table but may also use the polynomial as input and obtain its truth table, i.e. a full evaluation on its input-space.
\td{Finish subsection}
\subsubsection{Transformations on partial data}
...
\begin{figure}[h]
    ...
    \begin{alg}
        ...
        $U_0 \gets$ MOB\_TRANSFORM($V$[$0\dots |W^{n - n_1}_{w}| - 1$])\; \label{alg:output:mob_0}
        \ForEach{$i = 1\dots n_1$}{
            $U_i \gets$ MOB\_TRANSFORM($ZV$[$i$][$0, \dots, |W^{n - n1}_{w + 1}| - 1$])\; \label{alg:output:mob_1}
        }
        $Evals[0\dots n_1][0\dots 2^{n - n1} - 1] \gets \{0\}$\;
        \ForEach{$i = 0\dots n_1$}{
            $Evals[i][0\dots 2^{n - n1} - 1] \gets$ MOB\_TRANSFORM($U_i$.as\_array())\; \label{alg:output:mob_2}
        }
